...
const BootScreen = () => {
    ...
    useEffect(() => {
        const bootSequence = async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve, isBootScreen ? 1000 : 300))

            setBootStage("logo")
            await new Promise((resolve) => setTimeout(resolve, isBootScreen ? 2000 : 1000))

            setBootStage("progress")
            await new Promise((resolve) => setTimeout(resolve, isBootScreen ? 4000 : 3000))
    ...
    return (
        <div className="w-full h-full bg-black rounded-[50px] overflow-hidden relative flex items-center justify-center">
            <AnimatePresence mode="wait">
                ...
                {(bootStage === "logo" || bootStage === "progress" || bootStage === "complete" && !isShutdown) && (
                    <motion.div
                        key="boot"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                    ...
                    >
                        ...
                        >
                            ...
                            <motion.div
                                ...
                                className="absolute inset-0 bg-white rounded-full blur-xl scale-150"
                            />
                        </motion.div>
                    </motion.div>
                )}
